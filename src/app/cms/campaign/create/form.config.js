...
export const useInitialValues = (
  ...
  useEffect(() => {
    const makeInitialValues = () => {
      ...
      const formattedStartTime = formatTime(singleCampaign?.data?.startTime);
      const formattedEndTime = formatTime(singleCampaign?.data?.endTime);

      const { discountInAmount, discountInPercent } =
        singleCampaign?.data || {};

      let discountType = "";
      if (discountInAmount) {
        discountType = "amount";
      } else if (discountInPercent) {
...
export const UpdateValidationSchema = yup.object().shape({
  ...
  discountInPercent: yup
    .number()
    .nullable()
    .typeError("Discount in percent must be a number")
    .min(0, "Discount in percent cannot be less than 0")
    .max(100, "Discount in percent cannot be more than 100"),

  discountInAmount: yup
    .number()
    .nullable()
    .typeError("Discount in amount must be a number"),
  cityId: yup
    .array()
    .of(
      yup
